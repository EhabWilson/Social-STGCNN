...
class social_stgcnn27(nn.Module):
    ...
    def forward(self, v, a):
        ...
        att = torch.bmm(q, k.transpose(-2, -1))
        att = torch.relu(att) ** 2 / self.var1 / res.shape[1]
        ...
        # v = res + v
        # res = v.reshape(self.var1, 5, -1).permute(1, 2, 0)
        u1 = self.Wu(res)
        u2 = self.Wv(self.hidden_dict)
        u2 = self.act(u2)
        # print(att.size(), u1.size(), u2.size())
        u2 = torch.bmm(att, u2)
        res = self.Wo(u1 * u2)
        res = res.permute(2, 0, 1).reshape(1, 12, 5, -1).contiguous()
