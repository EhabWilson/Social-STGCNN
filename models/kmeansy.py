...
def test(loader_test, model, writer=None, epoch=0, KSTEPS=20):
    ...
    for batch in loader_test:
        ...
        # print(feature.size(), V_tr.size())

        # print(feature.size())

        feature_list.append(V_tr.permute(0, 2, 1, 3).reshape(-1, 24))
        # print(V_pred.shape)
        # torch.Size([1, 5, 12, 2])
        # torch.Size([12, 2, 5])
...
def process(args):
    ...
    for tag in DOMAINS:
        ...
        dset_test = TrajectoryDataset_Real(data_set + 'val/',
                                           obs_len=obs_seq_len,
        ...
        centers_init, indices = kmeans_plusplus(feature,
                                                n_clusters=n,
                                                random_state=np.random.randint(
                                                    0, 1000))

        centers_init = torch.from_numpy(centers_init).reshape(
            -1, 12, 2)
        if not os.path.exists("./ext_f/kmeans-valid-y-{}".format(n)):
            os.makedirs("./ext_f/kmeans-valid-y-{}".format(n))
        torch.save(centers_init, "./ext_f/kmeans-valid-y-{}/{}.pkl".format(n, tag))
