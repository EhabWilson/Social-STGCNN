...
class social_stgcnn21(nn.Module):
    ...
    def __init__(self,
        ...
        hidden_dict = hidden_dict.unsqueeze(0).permute(0, 3, 1, 2)
        self.hidden_dict = nn.Parameter(hidden_dict)
        self.proj = nn.Conv2d(pred_seq_len, pred_seq_len, 3, padding=1)
    ...
    def forward(self, v, a):
        ...
        # hidden = self.hidden_dict
        hidden = self.proj(self.hidden_dict).squeeze(0).permute(1, 2, 0)
        q = self.W1(res)
        k = self.W2(hidden)
