...
class social_stgcnn8(nn.Module):
    ...
    def __init__(self,
        ...
        hidden_dict = torch.randn(dict_len, 60) / math.sqrt(50)
        self.hidden_dict = nn.Parameter(hidden_dict)
        self.norm = nn.LayerNorm(var1)
        self.W1 = nn.Linear(60, var1)
        self.W2 = nn.Linear(60, var1)
        self.W3 = nn.Linear(60, var1)
        self.W4 = nn.Linear(var1, 60)
    ...
    def forward(self, v, a):
        ...
        a parameterized tensor \in R^{dict_len*60}
        v \in R^{1*12*5*num_person} -> v^T \in R^(num_person*60)
        ...
        """
        res = v
        res = res.reshape(-1, res.shape[-1]).T.contiguous()
        q = self.W1(res)
        k = self.W2(self.hidden_dict)
        hidden = self.W3(self.hidden_dict)
        att = torch.mm(q, k.T) / math.sqrt(self.var1)
        att = att.softmax(dim=-1)

        res = torch.mm(att, hidden)
        res = res.norm(res)
        res = self.W4(res)
        res = res.T.reshape(1, 12, 5, -1).contiguous()
