...
class social_stgcnn15(nn.Module):
    ...
    def __init__(self,
        ...
        self.hidden_dict = nn.Parameter(torch.FloatTensor(features))
        # self.hidden_dict = nn.Parameter(hidden_dict)
        self.drop = nn.Dropout(drop)
        self.W1 = nn.Linear(12, var1)
        self.W2 = nn.Linear(12, var1)
        self.Wa = nn.Linear(12, var1)
        self.Wb = nn.Linear(12, var1)
        self.Wall = nn.Linear(var1, 1)
    ...
    def forward(self, v, a):
        ...
        # attention 1
        q = self.Wa(res)
        q = q.reshape(q.shape[0], q.shape[1], 1, q.shape[2]).repeat(1, 1, self.hidden_dict.shape[1], 1)
        k = self.Wb(self.hidden_dict)
        k = k.reshape(k.shape[0], 1, k.shape[1], k.shape[2]).repeat(1, q.shape[1], 1, 1)
        ...
        ).reshape(res.shape[0], res.shape[1], self.hidden_dict.shape[1])
        ...
        att2 = torch.bmm(q, k.transpose(-2, -1)) / math.sqrt(self.var1)
        att2 = att2.softmax(dim=-1)

        hidden = self.W3(self.hidden_dict)

        res = torch.bmm(att1 + att2, hidden)
        res = self.W4(res)
