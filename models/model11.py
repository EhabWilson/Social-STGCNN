...
class social_stgcnn11(nn.Module):
    ...
    def __init__(self,
        ...
        features = torch.load("./ext_f/kmeans-valid-y-{}/{}.pkl".format(
            init, env))
        print("**", features.size())
        self.hidden_dict = nn.Parameter(torch.FloatTensor(features))
        self.hidden_dict.requires_grad = False
        self.dict_phy = nn.Linear(2, 5)
        self.dict_time = nn.Conv1d(pred_seq_len,
                                   pred_seq_len,
                                   kernel_size=dict_kernel_size,
                                   padding=1)
        # self.proj = nn.Conv2d(2, 5, kernel_size=3, padding=1)
    ...
    def forward(self, v, a):
        ...
        print(self.hidden_dict.size())
        hidden = self.dict_phy(self.hidden_dict.permute(1, 2, 0)).permute(
            2, 1, 0).contiguous()
        print(hidden.size())
        hidden = self.dict_time(hidden).permute(0, 2, 1)
