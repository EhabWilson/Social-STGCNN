...
class social_stgcnn19(nn.Module):
    ...
    def __init__(self,
        ...
        self.hidden_dict = nn.Parameter(torch.FloatTensor(features).permute(
            0, 2, 1))
        ...
        self.a1 = nn.Linear(12, var1)
        self.a2 = nn.Linear(12, var1)
        self.a3 = nn.Linear(12, var1)
        self.als = LayerScale(12)
        self.a4 = nn.Linear(var1, 12)
    ...
    def forward(self, v, a):
        ...
        # print(type(q), type(k))
        # print(k.size(), q.size())
        attn = torch.bmm(q, k.transpose(-2, -1)) / math.sqrt(self.var1)
        attn = attn.softmax(dim=-1)
        res = torch.bmm(attn, va)
        res = self.a4(res).permute(2, 0, 1).reshape(1, 12, 5, -1).contiguous()
        # print(res.size(), v.size())

        print(torch.sum(a), torch.sum(attn), a.size(), attn.size())
        # tensor(1.4672, device='cuda:0') tensor(50., device='cuda:0', grad_fn=<SumBackward0>) torch.Size([8, 10, 10]) torch.Size([5, 10, 10])
