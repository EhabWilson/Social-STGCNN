...
class social_stgcnn29(nn.Module):
    ...
    def __init__(self,
        ...
        self.time_pe = nn.Conv2d(seq_len, pred_seq_len, 3, padding=1)
        self.phy_pe = nn.Conv2d(2, 5, 3, padding=1)
    ...
    def forward(self, v, a, v_abs):
        ...
        # postional embedding
        pe = self.time_pe(v_abs.permute(0, 3, 1, 2).contiguous())
        pe = self.phy_pe(pe.permute(0, 3, 2, 1).contiguous()).squeeze(0)

        print(q.size(), pe.size())

        k = self.W2(self.hidden_dict)
        hidden = self.W3(self.hidden_dict)
        att = torch.bmm(q + pe, k.transpose(-2, -1)) / math.sqrt(self.var1)
